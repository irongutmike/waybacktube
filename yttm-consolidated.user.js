// ==UserScript==
// @name         WayBackTube Enhanced (Consolidated)
...
class UIManager {
    ...
    class YouTubeTimeMachine {
        ...
        async getChannelVideos(channelId, channelName, endDate, forceRefresh = false) {
            ...
            if (!videos) {
                try {
                    ...
                    this.log('Failed to get fresh videos for channel:', channelId, error);
                    videos = [];
                }
            } else {
                this.stats.cacheHits++;
        ...
        async enhanceVideoPage(sidebar) {
            ...
            // Force clear any existing enhancements first
            const existingSections = sidebar.querySelectorAll('.tm-video-page-section');
...
// Generate recommendations if videos are available
if (videos.length > 0) {
    this.log('Generating watch page recommendations for:', videoTitle);
    this.log('Fetching fresh videos for channel:', channelId);
    ...
    this.log('No cached videos available, loading fresh videos...');
    ...
    this.createWatchNextSection(sidebar, []);
        ...
        getCurrentChannelName() {
            ...
            return channelElement ? channelElement.textContent.trim() : '';
                    ...
                    this.log('Found', freshVideos.length, 'fresh videos from current channel');
            ...
            if (channelLink) {
                ...
                if (href) {
                    const match = href.match(/\/(channel|c|user)\/([^\/]+)/);
                        this.log('Found', seriesVideos.length, 'series videos matching keywords');
                    if (match) {
                        return match[2];
